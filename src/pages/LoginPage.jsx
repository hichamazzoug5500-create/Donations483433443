  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (res.needsCompletion) {
        ...
        showSuccess(lang === 'ar' ? 'مرحباً بك مجدداً!' : 'Welcome back!');
        navigate(res.role === 'recipient' ? '/dashboard' : '/donor');
      }
    } catch (err) {
      console.error("Google sign in error:", err);
      if (err.code === 'auth/unauthorized-domain') {
        setError('Please add this domain to the Authorized Domains in Firebase Authentication settings.');
      ...
      <div className="max-w-md w-full bg-white rounded-2xl border border-slate-200/90 shadow-sm p-8 space-y-6 text-center">
        ...
        <div className="space-y-3 pt-2">
          ...
          >
            ...
            <span>{isSubmitting ? t('connectingGoogle') : t('googleSignInBtn')}</span>
          </button>
          
          <p className="text-[11px] text-slate-400">
            {t('googleAuthVerifiedNote')}
          </p>
        </div>
