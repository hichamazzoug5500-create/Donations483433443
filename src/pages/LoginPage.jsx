...
export const LoginPage = () => {
  ...
  const { loginWithGoogle, loginDemoRole, currentUser, isProfileComplete, role: userRole } = useAuth();
  const { t } = useLanguage();
  ...
  const handleDemoClick = (role) => {
    loginDemoRole(role);
    showSuccess(role === 'recipient' ? 'تم الدخول في الوضع التجريبي كجمعية محتاجة' : 'تم الدخول في الوضع التجريبي كجهة متبرعة');
    navigate(role === 'recipient' ? '/dashboard' : '/donor');
  };
      ...
      <div className="max-w-md w-full bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden space-y-6 p-7 sm:p-8">
        ...
        <div className="space-y-4 pt-2">
          ...
          >
            ...
            <span>{isSubmitting ? '...' : t('googleSignInBtn')}</span>
          </button>
        ...
        {/* Demo Fast Logins as subtle pills */}
        <div className="pt-3 border-t border-slate-100">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className="py-2 px-3 bg-slate-50 hover:bg-amber-50 border border-slate-200 text-slate-700 hover:text-amber-900 rounded-xl text-xs font-semibold transition-colors truncate text-center"
            >
              {t('demoRecipientBtn')}
            </button>
            <button
              onClick={() => handleDemoClick('donor')}
              className="py-2 px-3 bg-slate-50 hover:bg-teal-50 border border-slate-200 text-slate-700 hover:text-teal-900 rounded-xl text-xs font-semibold transition-colors truncate text-center"
            >
              {t('demoDonorBtn')}
            </button>
          </div>
